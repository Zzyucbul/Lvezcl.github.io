...
public class SysUserService implements BaseService<TsysUser, TsysUserExample>{
	...
	/**
	 * 添加管理员和超级管理员如果是后者其schoolid是"0000"
	 * @param record
	 * @param roles
	 * @return
	 */
	@Transactional
	public int insertUserRoles(TsysUser record) {
		String userid=SnowflakeIdWorker.getUUID();
		record.setId(userid);
		String role = record.getRole();
		if (role.equals("889175176154058752")) {
			record.setSchoolId("0000");
		}
		if(StringUtils.isNotEmpty(record.getRole())){
			List<String> list_roles=ConvertUtil.toListStrArray(role);
